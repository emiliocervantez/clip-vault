...
Start with `--trace` to write a timestamped log of hotkeys, focus changes, clipboard reads and writes,
and any stall of ClipVault's own UI thread:

```
.\publish\ClipVault.exe --trace
```

The log is `%LOCALAPPDATA%\ClipVault\trace.log`. Tracing is off unless the flag is given.
